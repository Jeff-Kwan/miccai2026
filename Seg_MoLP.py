...
load_dir = "results/2026_02_02/08_42_MoLP"
output_dir = os.path.join(load_dir, "LVSeg")
os.makedirs(output_dir, exist_ok=True)

model = MotionLatentPerceiver(in_c=3, out_c=1, init_c=8, latent=256, 
                              enc_layers=2, t_layers=12, t_heads=4, t_latents=4, 
                            dec_layers=2, levels=4, 
                            motion_dim=2,
                            masking_ratio=0.75, skips=False)
pretrained = torch.load(os.path.join(load_dir, "MoLP.pth"), map_location=device)
...
comments = [
    f"Full Fine Tune Motion Latent Perceiver, {epochs} epochs no augmentations",
    "1 latent for motion 1 latent for centroid",
