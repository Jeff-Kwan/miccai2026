    ...
    with torch.inference_mode():
        for videos, timestamps, frames_idx, fps in tqdm(dl, desc=split_name):
            ...
            with torch.autocast('cuda', torch.bfloat16, enabled=use_amp):
                ...
                gcls, enc_tokens, _ = jepa.encoder(videos, keep_idx=keep_idx, timestamps=timestamps)
                frame_cls = enc_tokens[:, :, 0, :]
                _, z_motion = jepa.low_rank_latent(gcls, frame_cls, T)
                
            z_motion[:, 0].fill_(0.0)  # Zero out first dimension?
