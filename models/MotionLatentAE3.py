...
class MotionLatentAE(nn.Module):
    def __init__(self, in_c=3, out_c=3, latent=512, enc_layers=6, dec_layers=2, levels=6, skips=False):
        ...
        self.motion_basis = nn.Parameter(torch.randn(latent*2, 2) * 0.01)
        

    def forward(self, x):
        z, skips = self.encoder(x)

        # Low Rank next frame prediction
        X, _ = torch.linalg.qr(self.motion_basis)
        z_prev = z[:, :, :-1, :, :] if z.size(2) > 1 else z
        delta_z = self.motion_mlp(z_prev.squeeze(-1).squeeze(-1).transpose(1, 2)) @ X.T
        z_pred = z_prev + delta_z.transpose(1, 2).view_as(z_prev)
        
        with torch.no_grad():
            self.z_motion = (z - z.mean(dim=2, keepdim=True)).view(z.size(0), z.size(1), z.size(2)).transpose(1, 2) @ X

        if self.skips:
            return self.decoder(z, skips)
        else:
            return self.decoder(z, skips=None), self.decoder(z_pred, skips=None)
