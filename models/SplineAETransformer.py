...
class SplineAutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.bottleneck = TemporalTransformer1D(TsTransformerCfg(dim=latent, depth=4))
...
if __name__ == "__main__":
    ...
    with torch.profiler.profile(
        ...
        recon = model.forward_spline(
            x,
            torch.linspace(0, 1, steps=T, device=device).unsqueeze(0).expand(B, T),
            torch.linspace(0, 1, steps=T, device=device).unsqueeze(0).expand(B, T)
        )
