...
video = val_ds[idx]['video'].to(device).transpose(0, 1).unsqueeze(0)  # [1, T, C, H, W]
frames_idx = val_ds[idx]['frame_indices']

def select_clip(video, frames_idx, max_frames):
    """
    ...
    if span <= max_frames:
        ...
        return video[:, start:end], frames_idx_clip, (start, end)
...
video, frames_idx, _ = select_clip(video, frames_idx, max_frames)

# Reconstruction
_, C, T, H, W = video.shape
with torch.inference_mode():
    out = mae(video, return_frame_pred=True)
