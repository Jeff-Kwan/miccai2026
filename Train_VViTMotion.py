...
# 0.5 masking! Focus on motion reconstruction more?
mae = VideoMotionMAE(enc, dec, frame_dec, motion_dim=2, norm_pix_loss=False, mask_ratio=0.5)
mae = mae.to(device)
...
def plot_recons(mae, val_ds, output_dir, device, clip_len=16, stride=2):
    ...
    with torch.no_grad():
        ...
        pred = mae(clip_batch, return_pred=True, return_frame_pred=True)["pred_frames"]
...
for epoch in range(epochs):
    ...
    for batch in p_bar:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=autocast):
            loss = mae(aug_videos, target=videos, return_pred=False)["loss_frame"]
    ...
    with torch.no_grad():
        ...
        for batch in p_bar:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=autocast):
                loss = mae(videos, return_pred=False)["loss_frame"]
